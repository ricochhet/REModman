...
public class Writer
{
	...
	public void SetEndian(Endian EndianType)
	{
		CurrentEndian = EndianType;
	}
	...
	{
		Write(Encoding.BigEndianUnicode.GetBytes(Value), StartIndex, checked(Length * 2));
	}
	...
	{
		Write(Conversions.Int24ToByteArray(checked((int)Value)), 0, 3, EndianType);
	}
	...
	{
		Write(Conversions.Int40ToByteArray(checked((long)Value)), 0, 5, EndianType);
	}
	...
	{
		Write(Conversions.Int48ToByteArray(checked((long)Value)), 0, 6, EndianType);
	}
	...
	{
		Write(Conversions.Int56ToByteArray(checked((long)Value)), 0, 7, EndianType);
	}
