...
public class Conversions
{
	...
	{
		return (uint)(Buffer[4] | Buffer[3] | Buffer[2] | Buffer[1] | Buffer[0]);
	}

	public static long ByteArrayToInt48(byte[] Buffer)
	{
		return (uint)(Buffer[5] | Buffer[4] | Buffer[3] | Buffer[2] | Buffer[1] | Buffer[0]);
	}

	public static long ByteArrayToInt56(byte[] Buffer)
	{
		return (uint)(Buffer[6] | Buffer[5] | Buffer[4] | Buffer[3] | Buffer[2] | Buffer[1] | Buffer[0]);
	}
	...
	public static byte[] HexToByteArray(string Value)
	{
		...
		checked
		{
			...
			for (int i = 0; i <= num; i++)
			{
				array[i] = (byte)Math.Round(Math.Round(Conversion.Val("&h" + Value.Substring(i * 2, 2))));
			}
			return array;
		}
	}
